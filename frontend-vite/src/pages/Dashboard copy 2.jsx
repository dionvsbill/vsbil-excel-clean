...
async function parseResponse(res) {
  ...
  if (/<!DOCTYPE|<html/i.test(text)) {
    //console.error('Unexpected HTML response from server:', text);
  ...
  const filteredSheets = useMemo(() => {
    ...
    const pinned = lastEditedSheet && base.includes(lastEditedSheet) ? lastEditedSheet : null;
    const rest = base.filter((s) => s !== pinned);
  ...
  }, [sheets, searchQuery, lastEditedSheet]);
 ...
 async function saveAllChanges() {
  ...
  async function addSheet() {
    ...
    if (!j.error) {
      ...
      // Immediately preview to avoid "sheet not found"
  ...
  const addRowAbove = () => {
    ...
    if (!canEdit) return;
    if (!hot) {
      // If grid is empty, initialize a minimal 1x1
...
async function downloadSheet() {
  ...
  try {
    setLoading(true);
    const headers = await authHeader();
    // you need a backend XLSX route; adjust once implemented
  ...
  async function onUpgradeSuccess() {
    ...
    // Reload latest to refresh pinned
    await loadLatestSheet();
  }
...
const [showFormulaHelp, setShowFormulaHelp] = useState(false); // <-- add this
...
const [selectedHelp, setSelectedHelp] = useState(''); // <-- add this
...
const formulaDescriptions = {
  ...
  SUBTRACT: 'Subtracts one cell or range from another.', // <-- add this
};
...
// Decide height based on screen width
if (window.innerWidth >= 600) {
  // Desktop and larger → fixed height
  initialHeight = 350;
} else {
  // Mobile/tablet → full viewport height minus a little allowance
  ...
  hotRef.current = new Handsontable(container, {
    ...
    stretchH: 'none',
    width: '100%',
    height: initialHeight, // ✅ fixed bucket height
...
const applyFormula = () => {
  ...
  // --- Aggregate formulas (single input column) ---
  if (selectedFormula === 'SUM' && inputs.length === 1) {
    ...
    // Aggregate multiplication: use PRODUCT
    const startRow = 1;
    const endRow = stop - 1;
    const targetRow = stop;
    ...
    // Aggregate division: divide first cell by product of the rest
    const startRow = 1;
    const endRow = stop - 1;
    const targetRow = stop;
    ...
    // Aggregate subtraction: subtract sum of rest from first
    const startRow = 1;
    const endRow = stop - 1;
    const targetRow = stop;
    ...
    // --- Row-wise formulas (multiple input columns) ---
...
{showFormulaHelp && (
  ...
  >
    ...
    <div style={{ marginTop: 16, textAlign: 'right' }}>
      ...
      {/* Handsontable container: only this scrolls */}
      <div
